...
R_file = fopen('borghi_train_h2.txt'); %<<-- Change this file name with the file you want evaluate

Raw_Results = [labels,cell(16,1)];
DelayFromGT = zeros(17,2);
for (i = 1:16) Raw_Results{i,2} = zeros(1,7); end
%Format [label, [total, correct, missed, FP, Delay, FrameBasedDR]]
                ...
                if (strcmp(label_A,label_R)) %Check if the label 
                    if (end_R-start_R <= (end_A-start_A)*max_dim_ratio) %Checking if the gesture in the result is within the maximum dimention limits
                        if((min([end_A, end_R])-max([start_A, start_R]))/(end_A-start_A)>= min_overlap_ratio) %Checking if the overlap between the gesture in the result and the annotated one is bigger than the min overlap ratio
                            ...
                            DelayFromGT(ind,1) = DelayFromGT(ind,1)+str2double(R{gR+3}) - start_A;
                            DelayFromGT(ind,2) = DelayFromGT(ind,2)+str2double(R{gR+3}) - end_A;
                            found = true;
                        else
...
%Compact results will show 3 columns for each class+total: Detection rate, False positives ratio, Jaccard Index
...
%DelayFromGT(1:16,1) = DelayFromGT(1:16,1)./tmp(1:16,2);
%DelayFromGT(17,1) = nansum(DelayFromGT(1:16,1))/16;
%DelayFromGT(1:16,2) = DelayFromGT(1:16,2)./tmp(1:16,2);
%DelayFromGT(17,2) = nansum(DelayFromGT(1:16,2))/16;


%FPALL = [FPALL Compact_Results(17,2)];
end
